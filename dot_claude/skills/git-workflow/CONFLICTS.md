# Git Conflict Resolution — Reference

Identifying, resolving, and preventing merge conflicts. For when `git merge` or `git rebase` reports `CONFLICT`, when files show `<<<<<<<` markers, or when planning to avoid conflicts on a long-running branch.
